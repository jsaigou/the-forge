#!/bin/bash
# /usr/local/lib/forge/start-a1.sh
# Static launcher for the A1 (formerly "primary") inference slot (port 8080).
# All model-specific values are injected by the engine via:
#   EnvironmentFile=/etc/sysconfig/forge-a1-env   (KEY=VALUE pairs)
#   /etc/sysconfig/forge-a1-args                  (one extra arg per line)
#   FORGE_LLAMA_BIN (optional) — override llama-server path
# SELinux: chcon -t bin_t /usr/local/lib/forge/start-a1.sh
set -euo pipefail

ARGS_FILE=/etc/sysconfig/forge-a1-args
