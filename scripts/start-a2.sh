#!/bin/bash
# /usr/local/lib/forge/start-a2.sh
# Static launcher for the A2 (formerly "secondary") inference slot (port 8081).
# All model-specific values injected by the engine — same pattern as start-a1.sh.
#   FORGE_LLAMA_BIN (optional) — override llama-server path
# SELinux: chcon -t bin_t /usr/local/lib/forge/start-a2.sh
set -euo pipefail

ARGS_FILE=/etc/sysconfig/forge-a2-args
