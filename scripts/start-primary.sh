#!/bin/bash
# /usr/local/lib/forge/start-primary.sh
# Static launcher for the primary inference slot (port 8080).
# All model-specific values are injected by the engine via:
#   EnvironmentFile=/etc/sysconfig/forge-primary-env   (KEY=VALUE pairs)
#   /etc/sysconfig/forge-primary-args                  (one extra arg per line)
#   FORGE_LLAMA_BIN (optional) — override llama-server path
# SELinux: chcon -t bin_t /usr/local/lib/forge/start-primary.sh
set -euo pipefail

ARGS_FILE=/etc/sysconfig/forge-primary-args
